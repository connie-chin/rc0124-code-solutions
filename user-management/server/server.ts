...
app.post('/api/auth/sign-up', async (req, res, next) => {
  // first part of user-management: complete registration
  try {
...
app.post('/api/auth/sign-in', async (req, res, next) => {
  try {
    ...
    const verified = await argon2.verify(hashedPassword, password); // confirm what they used to sign in matches database. a boolean
    if (!verified) throw new ClientError(401, 'invalid login'); // pw they used to log in is incorrect
    const payload = { userId, username }; // creating properties w these names, and assigns the contents to it. userId:userId
    const token = jwt.sign(payload, hashKey); // hashkey from up there
...
app.post('/api/todos', authMiddleware, async (req, res, next) => {
  // inline middleware that only applies at this endpoint
  try {
