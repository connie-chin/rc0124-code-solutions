...
function capitalizeWords(string) {
  let stringToArray = string.split(' '); //creates a new array of words from the sentence, separated by ' '
  let refinedArray = [];
  let result = '';
  for (let i = 0; i < stringToArray.length; i++) {
    //loops through the array of words I have created
    let str = stringToArray[i];
    let firstLetter = str.charAt(0).toUpperCase();
    let restOfWord = str.substring(1).toLowerCase();
